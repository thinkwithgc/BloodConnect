...
| Commit | What |
|---|---|
| `26d32bf` | **Every magic-link route joins the service-worker denylist** — an invitee tapped Activate and got the password field alone, because the handset ran a precached shell older than `a11192b`. Same bug as `7d1def8`, second membership rule: `/setup/` `/activate/` `/consent/` `/camp/` `/alert/` |
...
**The first real invitation appeared to not have the feature** — the invitee got the
password field alone. That was **not** this change but the precached-shell bug, on
`/activate/` this time, fixed in `26d32bf`; every check of the username seam came back
clean (bundle, route, no skew). **So it is still not exercised by a real person** — a
handset that was already broken needs **one reload** to pick up the new service worker,
and the walk-through is the outstanding step. See **Staff pick their OWN username at
setup** and **A precached SPA shell is a shell from a DIFFERENT BUILD**.
...
- **[A precached SPA shell is a shell from a DIFFERENT BUILD](docs/Raktify_Engineering_Lessons.md#a-precached-spa-shell-is-a-shell-from-a-different-build-2026-09-04)** — **TWO** membership rules for Workbox's `navigateFallbackDenylist`, and it recurred on 2026-09-08 (`26d32bf`) because only the first was known: (1) any path a **managed function rewrites**, or a precache hit never reaches the function; (2) every **one-shot / magic-link entry point** — `/setup/`, `/activate/`, `/consent/`, `/camp/`, `/alert/` — because the person arrives from WhatsApp on a device holding an arbitrarily old shell and acts **once**, so a field shipped later is silently absent and often unrecoverable (there is **no username-change endpoint**, so a stale-shell setup locks in the derived name; recovery is an admin `reissue-setup`). **Adding a new token route means adding it here.** When a shipped element is missing on real devices and present in a private window, suspect the **service worker** before the CSS — and note the fix needs **one reload** on an already-broken handset, since the navigation that finds the new `sw.js` is still served by the old worker.
