...
**EXERCISED BY A REAL INVITEE ON A REAL HANDSET, 2026-09-09 — this seam is done.** It
took a detour to get there: the first invitation appeared to not have the feature at
all (the invitee got the password field alone), which was **not** this change but the
precached-shell bug on `/activate/`, fixed in `26d32bf`. Every check of the username
seam came back clean throughout (bundle, route, no skew), and after the one reload an
already-broken handset needs, the invitee picked their own username and it worked first
time. See **Staff pick their OWN username at setup** and **A precached SPA shell is a
shell from a DIFFERENT BUILD**.
